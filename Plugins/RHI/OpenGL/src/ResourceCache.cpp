...
std::shared_ptr<StaticMesh> ResourceCache::cacheMesh(const std::string& key, MeshData data) {
    ...
    auto mesh = std::make_shared<StaticMesh>(gpuUploadEnabled_ ? StaticMesh::Upload(data)
                                                               : StaticMesh::CreateCpu(data));
    if (!mesh->Valid()) {
        return nullptr;
    }
...
std::shared_ptr<StaticMesh> ResourceCache::LoadStaticMesh(const std::string& path) {
...
std::shared_ptr<Texture> ResourceCache::LoadTexture(const std::string& path) {
    ...
    auto texture = std::make_shared<Texture>(Texture::LoadFromFile(path));
    if (!texture->Valid()) {
        return nullptr;
    }
...
std::shared_ptr<EnvMap> ResourceCache::LoadEnvMap(const std::string& path, int faceSize) {
    if (!gpuUploadEnabled_) {
        return nullptr;
    }
    ...
    auto env = std::make_shared<EnvMap>(EnvMap::LoadFromHdr(path, faceSize));
    if (!env->Valid()) {
...
std::shared_ptr<Texture> ResourceCache::CheckerTexture(int size) {
    if (!gpuUploadEnabled_) {
        return nullptr;
    }
    size = std::max(size, 2);
    ...
    auto texture = std::make_shared<Texture>(Texture::CreateChecker(size));
    if (!texture->Valid()) {
        return nullptr;
    }
    textures_.emplace(cacheKey, texture);
...
std::shared_ptr<Texture> ResourceCache::BumpNormalTexture(int size) {
    if (!gpuUploadEnabled_) {
        return nullptr;
    }
    size = std::max(size, 8);
    ...
    auto texture = std::make_shared<Texture>(Texture::CreateBumpNormal(size));
    if (!texture->Valid()) {
        return nullptr;
    }
    textures_.emplace(cacheKey, texture);
...
Material ResourceCache::LoadMaterial(const std::string& path) {
    const std::string cacheKey = normalizeKey(path);
...
Material ResourceCache::DefaultMaterial() {
...
std::shared_ptr<StaticMesh> ResourceCache::GetCubeMesh() {
    return cacheMesh("proc:cube", MakeCube());
}

std::shared_ptr<StaticMesh> ResourceCache::GetPlaneMesh(float size, float uvScale) {
    const std::string key = "proc:plane:" + std::to_string(size) + ":" + std::to_string(uvScale);
    return cacheMesh(key, MakePlane(size, uvScale));
}

std::shared_ptr<StaticMesh> ResourceCache::GetSphereMesh(int segments, int rings) {
...
void ResourceCache::InvalidateMaterial(const std::string& path) {

@echo off
REM Engine\Build\BatchFiles\GetVSEnv.bat — shared MSVC env + CMake PATH for Leon batch files.
REM Call from another script (after setlocal / cd to repo root):
REM   call "%~dp0GetVSEnv.bat" vcvars
REM   call "%~dp0GetVSEnv.bat" vsdev
...
if not defined _LEON_MODE (
  echo ERROR: GetVSEnv.bat needs mode vcvars or vsdev
  exit /b 1
)

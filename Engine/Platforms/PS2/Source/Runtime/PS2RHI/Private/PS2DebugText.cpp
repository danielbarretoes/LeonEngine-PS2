...
qword_t* AppendGlyphRuns(Leon::PS2::FPS2GSContext& gs, qword_t* q, float x, float y, char ch, float cell,
                         const color_t& color) {
...
void FPS2RHI::DrawDebugText(float x, float y, const char* text, float r, float g, float b,
                         float scale) {
    if (text == nullptr) {
        return;
    }

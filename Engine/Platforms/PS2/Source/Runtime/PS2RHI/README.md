...
1. `FPS2RHI::InitDisplay(Width, Height)`: VRAM framebuffer + 32-bit z-buffer, CRTC, draw environment. Done by
   `FPS2Window::Create()` (640x448 by default); games do not call it.
2. `SetViewTarget()` / `SetDirectionalLight()` / `SetAmbientLightColor()`.
3. `ClearColor()` → `BindMaterial()` → `DrawBox()` ... → 2D / `DrawDebugText()` on top.
4. `FPS2RHI::WaitVSync()`: present. Done by `FPS2Window::SwapBuffers()` at the end of the `FEngineLoop` frame.
...
| Group | Functions |
| --- | --- |
| Display | `InitDisplay(int Width, int Height)`, `ClearColor(float R, float G, float B)`, `WaitVSync()` |
...
`PS2Texture.h`: `FPS2Texture` is a move-only GS VRAM texture (RGBA8 uploaded as `PSMCT32`).
...
| `Private/PS2DynamicRHI.cpp` | `FPS2DynamicRHI`, `PlatformCreateDynamicRHI()`, `InitDisplay`, `ClearColor`, `WaitVSync` |
| `Private/PS2GSContext.h/.cpp` | framebuffer, z-buffer, DMA packet, VRAM bookkeeping (`Leon::PS2::FPS2GSContext`) |
...
| `Private/PS2DrawPrimitives.cpp` | 2D unlit triangles / rects, `DrawCookedMesh` |

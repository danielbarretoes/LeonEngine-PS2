...
| API | Header | Role |
| --- | --- | --- |
| `UCookCommandlet::Main` | `Public/Commandlets/CookCommandlet.h` | `staticmesh` / `recipe` modes; returns a process exit code |
| `FCookRecipe::RunFile` | `Public/CookRecipe.h` | Runs a JSON recipe (`steps` of `staticmesh`) |
| `FCookPaths::ResolveBeside` | `Public/CookPaths.h` | Resolves recipe paths next to the recipe file |

Dependencies (`Cooker.Build.cmake`): public `Core`; private `MeshUtilities` (`FStaticMeshBuilder`), `Json` (recipe parsing).

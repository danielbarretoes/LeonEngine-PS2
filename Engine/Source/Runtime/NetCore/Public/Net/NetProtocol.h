...
#pragma pack(pop)
	...
	inline void WriteLevelKey(char (&Dest)[MaxLevelKeyBytes], std::string_view Key)
	{
		...
		const std::size_t N = Key.size() < (MaxLevelKeyBytes - 1) ? Key.size() : (MaxLevelKeyBytes - 1);
	...
	inline void SetInputButton(FInputCmdMsg& Cmd, EInputButton Button, bool bDown)
	{
		if (bDown)
		{
			Cmd.Buttons = static_cast<std::uint16_t>(Cmd.Buttons | static_cast<std::uint16_t>(Button));
		}
		else
		{
	...
	[[nodiscard]] inline bool EncodeRpc(std::vector<std::uint8_t>& Out, ERpcId InRpcId, std::uint8_t InTargetSlot,
		const void* Payload, std::uint16_t InPayloadBytes)
	{

...
bool FTransformLocalBoxTest::RunTest(const FString& Parameters)
{
	...
	TestTrue("Min Y kept", Box.Min.Y <= -50.0f + 1.0e-2f);
	TestTrue("Max Y kept", Box.Max.Y >= 50.0f - 1.0e-2f);
	return true;
}
...
bool FFrustumIntersectsAabbTest::RunTest(const FString& Parameters)
{
	...
	const FMatrix View = MakeLookAtView(FVector(0.0f, 0.0f, 500.0f), FVector(0.0f), FVector(0.0f, 1.0f, 0.0f));
	...
	TestFalse("Box behind the camera",
		Frustum.IntersectsAabb(FBox(FVector(-50.0f, -50.0f, 600.0f), FVector(50.0f, 50.0f, 700.0f))));
	TestFalse("Box before the near plane",
		Frustum.IntersectsAabb(FBox(FVector(-1.0f, -1.0f, 495.0f), FVector(1.0f, 1.0f, 496.0f))));
	TestFalse("Box past the far plane",
		Frustum.IntersectsAabb(FBox(FVector(-50.0f, -50.0f, -9700.0f), FVector(50.0f, 50.0f, -9600.0f))));
	return true;
}

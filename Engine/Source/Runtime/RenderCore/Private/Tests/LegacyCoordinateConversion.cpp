...
void FLegacyCoordinateConversion::ComputeLegacyTangents(FMeshData& Data)
{
	// Frozen as the legacy renderer had it (glm::normalize: no tolerance).
	const auto Normalize = [](const FVector& V) { return V * (1.0f / FMath::Sqrt(V | V)); };
	if (Data.IsEmpty())
	{
		return;
	}
	...
	{
		const int32 Corners[3] = {static_cast<int32>(Data.Indices[I]), static_cast<int32>(Data.Indices[I + 1]),
			static_cast<int32>(Data.Indices[I + 2])};
		const FVertex& V0 = Data.Vertices[Corners[0]];
		const FVertex& V1 = Data.Vertices[Corners[1]];
		const FVertex& V2 = Data.Vertices[Corners[2]];
		...
		{
			TanAcc[Corner] += Tangent;
			BitAcc[Corner] += Bitangent;
		}
	}
	for (int32 I = 0; I < Data.Vertices.Num(); ++I)
	{
		...
		T = Normalize(T - (N * (N | T)));
		Vertex.Tangent = FVector4(T, (((N ^ T) | BitAcc[I]) < 0.0f) ? -1.0f : 1.0f);
	}
}

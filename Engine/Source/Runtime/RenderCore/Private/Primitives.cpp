...
FMeshData MakeSphere(int Segments, int Rings)
{
	...
	FMeshData Data;
	Data.Vertices.reserve(static_cast<std::size_t>(Rings + 1) * static_cast<std::size_t>(Segments + 1));
	...
	for (int Y = 0; Y <= Rings; ++Y)
	{
		...
		for (int X = 0; X <= Segments; ++X)
		{
			...
			Data.Vertices.push_back(FVertex{.Position = Normal * Radius, .Normal = Normal, .TexCoord = {U, 1.0f - V}});
		}
	}
	...
	for (int Y = 0; Y < Rings; ++Y)
	{
		for (int X = 0; X < Segments; ++X)
		{
			const auto I0 = static_cast<std::uint32_t>(
				(static_cast<std::size_t>(Y) * static_cast<std::size_t>(Segments + 1)) + static_cast<std::size_t>(X));

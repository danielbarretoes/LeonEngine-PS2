...
namespace
{
	...
	glm::vec3 SampleEquirect(const float* Data, int Width, int Height, const glm::vec3& Dir)
	{
		...
		{
			const std::size_t I =
				((static_cast<std::size_t>(Py) * static_cast<std::size_t>(Width)) + static_cast<std::size_t>(Px)) * 3u;
	...
	glm::vec3 ConvolveIrradiance(const float* Data, int Width, int Height, const glm::vec3& N)
	{
		...
		for (int IPhi = 0; IPhi < PhiSteps; ++IPhi)
		{
			...
			for (int ITheta = 0; ITheta < ThetaSteps; ++ITheta)
			{
				...
				const glm::vec3 TangentSample{SinTheta * std::cos(Phi), SinTheta * std::sin(Phi), CosTheta};
				const glm::vec3 SampleVec =
					glm::normalize(TangentSample.x * Right + TangentSample.y * Up + TangentSample.z * N);
	...
	unsigned int UploadCubeRgb16f(const std::vector<std::vector<float>>& Faces, int InFaceSize, bool bGenerateMips)
	{
		...
		{
			glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + FaceIndex, 0, GL_RGB16F, InFaceSize, InFaceSize, 0, GL_RGB,
				GL_FLOAT, Faces[static_cast<std::size_t>(FaceIndex)].data());
		}
...
FEnvironmentMap FEnvironmentMap::LoadFromHdr(const std::string& Path, int InFaceSize, int IrradianceSize)
{
	...
	for (int FaceIndex = 0; FaceIndex < 6; ++FaceIndex)
	{
		...
		for (int Y = 0; Y < InFaceSize; ++Y)
		{
			...
			{
				const float U = ((((static_cast<float>(X) + 0.5f) / static_cast<float>(InFaceSize)) * 2.0f) - 1.0f);
				const float V = ((((static_cast<float>(Y) + 0.5f) / static_cast<float>(InFaceSize)) * 2.0f) - 1.0f);
				const glm::vec3 Color = SampleEquirect(Data, Width, Height, FaceDirection(FaceIndex, U, V));
				const std::size_t I = ((static_cast<std::size_t>(Y) * static_cast<std::size_t>(InFaceSize)) +
	...
	std::cout << "EnvMap: convolving irradiance " << IrradianceSize << "^2 (may take a moment)...\n";
	std::vector<std::vector<float>> IrrFaces(6);
	for (int FaceIndex = 0; FaceIndex < 6; ++FaceIndex)
	{
		IrrFaces[static_cast<std::size_t>(FaceIndex)].assign(
			static_cast<std::size_t>(IrradianceSize) * static_cast<std::size_t>(IrradianceSize) * 3u, 0.0f);
		...
		for (int Y = 0; Y < IrradianceSize; ++Y)
		{
			for (int X = 0; X < IrradianceSize; ++X)
			{
				...
				const std::size_t I = ((static_cast<std::size_t>(Y) * static_cast<std::size_t>(IrradianceSize)) +
										  static_cast<std::size_t>(X)) *
					3u;
				Face[I + 0] = Color.r;
				Face[I + 1] = Color.g;
	...
	std::cout << "EnvMap: loaded '" << Path << "' -> cubemap " << InFaceSize << "^2 (" << Map.MipCount
			  << " mips) + irradiance " << IrradianceSize << "^2\n";
	return Map;
}

...
void AppendTextMesh(std::vector<FDrawVert>& Tris, const std::string& InText, float OriginX,
...
void AppendRightAlignedLines(std::vector<FDrawVert>& Tris, const std::string& InText,
                             int FramebufferWidth, float OriginY, float InPixelScale,
                             const std::array<unsigned char, 4>& InColor) {
    if (InText.empty()) {
        return;
    }
    ...
    while (Start <= InText.size()) {
        ...
        const std::size_t Count =
            (End == std::string::npos) ? (InText.size() - Start) : (End - Start);
        ...
        if (!Line.empty()) {
            ...
            const float OriginX =
                static_cast<float>(FramebufferWidth) - (RawWidth * InPixelScale) - MarginX;
...
void AppendCenterAlignedLines(std::vector<FDrawVert>& Tris, const std::string& InText,
                              int FramebufferWidth, float OriginY, float InPixelScale,
                              const std::array<unsigned char, 4>& InColor) {
    if (InText.empty()) {
        return;
    ...
    while (Start <= InText.size()) {
        ...
        if (!Line.empty()) {
            ...
            const auto RawWidth = static_cast<float>(stb_easy_font_width(MutableLine.data()));
            const float OriginX =
                (static_cast<float>(FramebufferWidth) - (RawWidth * InPixelScale)) * 0.5f;
            AppendTextMesh(Tris, Line, OriginX, LocalY, InPixelScale, InColor);
        }
...
void AppendJustifiedLines(std::vector<FDrawVert>& Tris, const std::string& InText, float AnchorX,
                          float OriginY, float InPixelScale, ETextJustify InJustify,
                          const std::array<unsigned char, 4>& InColor) {
    if (InText.empty()) {
        return;
    }
...
void AppendThickScreenLine(std::vector<FDrawVert>& Tris, float InX0, float InY0, float InX1, float InY1,
    ...
    AppendScreenQuad(Tris, InX0 - Hx, InY0 - Hy, InX0 + Hx, InY0 + Hy, InX1 + Hx, InY1 + Hy, InX1 - Hx, InY1 - Hy,
                     InColor);
}
...
void FDebugOverlay::TickOnScreenMessages(float DeltaTime) {
    ...
    const auto EraseIt =
        std::remove_if(OnScreenMessages.begin(), OnScreenMessages.end(),
...
void FDebugOverlay::AddScreenLine(float InX0, float InY0, float InX1, float InY1, const glm::vec3& InColor,
                                 float InThickness) {
...
void FDebugOverlay::AddScreenText(std::string InText, float InX, float InY, const glm::vec3& InColor,
                                 float InPixelScale, ETextJustify InJustify) {
    if (InText.empty()) {
        return;
    }
    ScreenTexts.push_back(
        FScreenText{std::move(InText), InX, InY, InPixelScale, InJustify, InColor});
    bDirty = true;
}

void FDebugOverlay::MeasureText(const std::string& InText, float InPixelScale, float& OutWidth,
                               float& OutHeight) {
    float MaxRaw = 0.0f;
    int Lines = 1;
...
void FDebugOverlay::RebuildMesh(int FramebufferWidth, int FramebufferHeight) {
    ...
    if (Vao == 0 ||
        (Text.empty() && BottomLeftText.empty() && CenterText.empty() && RightText.empty() &&
         OnScreenMessages.empty() && ScreenLines.empty() && ScreenRects.empty() &&
         ScreenTexts.empty())) {
        return;
    }
    ...
    if (!BottomLeftText.empty()) {
        ...
        const float OriginY = static_cast<float>(FramebufferHeight) - MarginY -
                              (LineStepY * static_cast<float>(LineCount));
    ...
    if (!CenterText.empty()) {
        ...
        OriginY = std::clamp(OriginY, MarginY,
                             std::max(MarginY, static_cast<float>(FramebufferHeight) - BlockH -
                                                    MarginY));
        // Each line centered — long Main Menu hints must not left-bias short rows.
        AppendCenterAlignedLines(Tris, CenterText, FramebufferWidth, OriginY, HudPixelScale,
                                 CenterColor);
    }

    // Right-aligned block (stats top-right / level chrome bottom-right).
    AppendRightAlignedLines(Tris, RightText, FramebufferWidth, RightTextOriginY, HudPixelScale,
                            RightColor);
    ...
    for (auto It = OnScreenMessages.rbegin(); It != OnScreenMessages.rend(); ++It) {
        ...
        AppendTextMesh(Tris, It->Text, MarginX, LocalY, MessagePixelScale,

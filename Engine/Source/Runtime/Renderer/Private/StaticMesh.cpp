...
StaticMesh StaticMesh::CreateCpu(const MeshData& data) {
    StaticMesh mesh;
    if (data.empty()) {
        return mesh;
    }
    ...
    if (data.submeshes.empty()) {
        mesh.submeshes_.push_back(SubMesh{0, mesh.indexCount_, 0});
    } else {
...
StaticMesh StaticMesh::Upload(const MeshData& data) {
    StaticMesh StaticMesh;
    if (data.empty()) {
        return StaticMesh;
    }

    MeshData uploadData = data;
    ComputeTangents(uploadData);

    StaticMesh.localMin_ = glm::vec3(std::numeric_limits<float>::max());
    StaticMesh.localMax_ = glm::vec3(std::numeric_limits<float>::lowest());
    for (const Vertex& vertex : uploadData.vertices) {
        StaticMesh.localMin_ = glm::min(StaticMesh.localMin_, vertex.position);
        StaticMesh.localMax_ = glm::max(StaticMesh.localMax_, vertex.position);
    }

    glGenVertexArrays(1, &StaticMesh.vao_);
    glGenBuffers(1, &StaticMesh.vbo_);
    glGenBuffers(1, &StaticMesh.ebo_);

    glBindVertexArray(StaticMesh.vao_);

    glBindBuffer(GL_ARRAY_BUFFER, StaticMesh.vbo_);
    glBufferData(GL_ARRAY_BUFFER,
                 static_cast<GLsizeiptr>(uploadData.vertices.size() * sizeof(Vertex)),
                 uploadData.vertices.data(), GL_STATIC_DRAW);

    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, StaticMesh.ebo_);
    ...
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex),
                          GlAttribOffset(&Vertex::normal));

    glEnableVertexAttribArray(2);
    glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex),
                          GlAttribOffset(&Vertex::texCoord));

    glEnableVertexAttribArray(3);
    glVertexAttribPointer(3, 4, GL_FLOAT, GL_FALSE, sizeof(Vertex),
                          GlAttribOffset(&Vertex::tangent));

    glBindVertexArray(0);
    StaticMesh.indexCount_ = static_cast<int>(uploadData.indices.size());
    StaticMesh.materials_ = uploadData.materials;

    if (uploadData.submeshes.empty()) {
        StaticMesh.submeshes_.push_back(SubMesh{0, StaticMesh.indexCount_, 0});
    } else {
        StaticMesh.submeshes_ = uploadData.submeshes;
    }
    StaticMesh.cpuData_ = std::move(uploadData);
    return StaticMesh;
}
...
void StaticMesh::DrawSubMesh(std::size_t subMeshIndex) const {
    if (!Valid() || cpuOnly_ || vao_ == 0 || subMeshIndex >= submeshes_.size()) {
        return;
    }
    const SubMesh& sub = submeshes_[subMeshIndex];
    if (sub.indexCount <= 0) {
        return;
    }
    glBindVertexArray(vao_);

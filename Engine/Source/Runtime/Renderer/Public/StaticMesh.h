...
public:
    ...
    [[nodiscard]] bool IsCpuOnly() const { return bCpuOnly; }
    [[nodiscard]] int GetIndexCount() const { return IndexCount; }
    [[nodiscard]] int TriangleCount() const { return IndexCount / 3; }
    [[nodiscard]] const glm::vec3& GetLocalMin() const { return LocalMin; }
    [[nodiscard]] const glm::vec3& GetLocalMax() const { return LocalMax; }
    [[nodiscard]] const std::vector<FMeshSection>& GetSubmeshes() const { return Submeshes; }
    [[nodiscard]] const std::vector<FMaterial>& GetMaterials() const { return Materials; }
    [[nodiscard]] bool HasMaterials() const { return !Materials.empty(); }
    /// CPU copy retained for lightmap bake / editor tools (empty if upload had no data).
    [[nodiscard]] const FMeshData& GetCpuData() const { return CpuData; }
    [[nodiscard]] bool HasCpuData() const { return !CpuData.empty(); }

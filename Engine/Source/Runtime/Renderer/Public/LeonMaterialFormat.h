...
[[nodiscard]] bool LoadLeonMaterialFile(FResourceCache& Resources, const std::string& Path, FMaterial& Out);

/// Write a `.lmat` from CPU material parameters (texture paths optional).
[[nodiscard]] bool SaveLeonMaterialFile(const std::string& Path, const std::string& InName, const FMaterial& InMaterial,
...
[[nodiscard]] std::string MakeDefaultLeonMaterialText(const std::string& InName,
	const glm::vec3& BaseColor = {0.7f, 0.7f, 0.72f}, float Metallic = 0.0f, float Roughness = 0.6f);

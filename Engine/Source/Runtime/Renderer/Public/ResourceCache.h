...
public:
    ...
    [[nodiscard]] std::shared_ptr<FEnvironmentMap> LoadEnvMap(const std::string& Path,
                                                     int FaceSize = FEnvironmentMap::DefaultFaceSize);
    ...
    void SetGpuUploadEnabled(bool bEnabled) { bGpuUploadEnabled = bEnabled; }
    [[nodiscard]] bool IsGpuUploadEnabled() const { return bGpuUploadEnabled; }

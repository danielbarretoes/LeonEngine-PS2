...
public:
    ...
    void SetCapsule(const FCapsuleShape& InCapsule) { Capsule = InCapsule; }
    void SetCharacterMovement(const UCharacterMovementComponent& InMovement) { Movement = InMovement; }

    [[nodiscard]] const FCapsuleShape& GetCapsule() const { return Capsule; }
    [[nodiscard]] UCharacterMovementComponent& GetCharacterMovement() { return Movement; }
    [[nodiscard]] const UCharacterMovementComponent& GetCharacterMovement() const { return Movement; }

    /// Unreal-like UCharacterMovementComponent::SetMovementMode / MovementMode.
    void SetMovementMode(EMovementMode NewMode);
    [[nodiscard]] EMovementMode GetMovementMode() const { return MovementMode; }

    /// Unreal-like UCharacterMovementComponent::IsMovingOnGround.
    [[nodiscard]] bool IsMovingOnGround() const { return MovementMode == EMovementMode::Walking; }
    /// Unreal-like UCharacterMovementComponent::IsFalling (airborne / jumping).
    [[nodiscard]] bool IsFalling() const { return MovementMode == EMovementMode::Falling; }
    /// Vertical velocity (Unreal Velocity.Z) for jump SM apex detection.
    [[nodiscard]] float GetVelocityZ() const { return VelocityY; }
    ...
    [[nodiscard]] const FFindFloorResult& GetCurrentFloor() const { return CurrentFloor; }
    ...
    [[nodiscard]] USkeletalMeshComponent& GetMesh() { return Mesh; }
    [[nodiscard]] const USkeletalMeshComponent& GetMesh() const { return Mesh; }

    /// Apply replicated movement state (client proxy / snapshot).
    void ApplyReplicatedState(const glm::vec3& Location, float YawDegrees, float InVelocityY,
                              bool bGrounded);

    /// Normalized locomotion blend input [0,1] for Mesh UAnimInstance UBlendSpace1D.
    void SetAnimBlendInput(float SpeedAlpha);
    [[nodiscard]] float GetAnimBlendInput() const { return AnimBlendInput; }

...
public:
    ...
    [[nodiscard]] UWorld& GetWorld() { return World; }
    [[nodiscard]] const UWorld& GetWorld() const { return World; }

    /// Recreate World FPhysScene backend (clears bodies). Prefer before RegisterBodiesFromLevel.
    void SetPhysicsBackend(EPhysicsBackend Backend) { World.SetPhysicsBackend(Backend); }

    [[nodiscard]] AGameStateBase& GetGameState() { return *GameState; }
    [[nodiscard]] const AGameStateBase& GetGameState() const { return *GameState; }
...
protected:
    AGameModeBase() : GameState(std::make_unique<AGameStateBase>()) {}

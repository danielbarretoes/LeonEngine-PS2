 ...
 *
 * The map is a long package name (`/Engine/LevelTemplates/Starter`, `/Game/Maps/X`), a file path or a legacy content
 * key (`LevelTemplates/Starter.llev`); a path that starts with a drive letter, or with `/` without being a long package
 * name, is a plain file name (UE's rule). Leon has no networking, so the protocol, host and port of UE's URL are left
 * out.
 */
USTRUCT()

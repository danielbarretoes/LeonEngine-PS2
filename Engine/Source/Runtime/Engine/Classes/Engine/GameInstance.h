...
public:
	...
	[[nodiscard]] UNetDriver& GetNetDriver()
	{
		return *NetDriver;
	}
	[[nodiscard]] const UNetDriver& GetNetDriver() const
	{
		return *NetDriver;
	}
	...
	{
		return GetNetMode() == ENetMode::ListenServer;
	}
	...
	{
		return GetNetMode() == ENetMode::Client;
	}
	[[nodiscard]] bool IsNetHost() const
	{
		return NetDriver->IsHost();
	}
	...
	[[nodiscard]] bool HasPendingDedicatedStart() const
	{
		return bPendingDedicatedStart;
	}
	void ClearPendingDedicatedStart()
	{
	...
	[[nodiscard]] std::uint16_t GetPendingDedicatedPort() const
	{
		return PendingDedicatedPort;
	}

	/// Optional `--join <ip>` from the app host; Menu consumes into session address.
	void SetPendingJoinAddress(std::string Address)
	{
		PendingJoinAddress = std::move(Address);
	}
	...
	[[nodiscard]] bool HasPendingListenStart() const
	{
		return bPendingListenStart;
	}
	...
	[[nodiscard]] std::uint16_t GetPendingListenPort() const
	{
		return PendingListenPort;
	}

	/// Optional `--map <LevelKey>` (Unreal Play current map): skip Lobby, travel to match.
	void SetPendingPlayMap(std::string MapKey)
	{
		PendingPlayMap = std::move(MapKey);
	}
	...
	[[nodiscard]] const std::string& PeekPendingPlayMap() const
	{
		return PendingPlayMap;
	}

	void SetLevelTravelFn(FLevelTravelFunction Fn)
	{
		LevelTravelFn = std::move(Fn);
	}
	/// Used by Engine::SetGameInstance to keep Runtime/Editor travel wiring across subclass swap.
	[[nodiscard]] FLevelTravelFunction TakeLevelTravelFn()
	{
		return std::move(LevelTravelFn);
	}
...
private:
	[[nodiscard]] bool TravelInternal(UGameEngine& Engine, std::string_view LevelKey, std::string_view HintLevelPath);

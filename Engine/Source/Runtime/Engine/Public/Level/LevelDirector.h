...
public:
    ...
    [[nodiscard]] bool IsEmpty() const { return Catalog.IsEmpty(); }
    [[nodiscard]] std::size_t GetCurrentIndex() const { return CurrentIndex; }
    [[nodiscard]] const FLevelCatalog& GetCatalog() const { return Catalog; }

    /// When false, `[`/`]` chrome and input are disabled (menus / shipping UI).
    void SetBrowserVisible(bool bVisible) { bBrowserVisible = bVisible; }
    [[nodiscard]] bool IsBrowserVisible() const { return bBrowserVisible; }

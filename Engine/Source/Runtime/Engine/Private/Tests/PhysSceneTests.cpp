...
{
	FPhysScene Scene;
	REQUIRE(Scene.GetBodies().Num() == 0);
	const int32 Id = Scene.AddBody({0, EBodyType::Static, 1.0f, true});
	REQUIRE(Id == 0);
	REQUIRE(Scene.GetBodies().Num() == 1);
	Scene.Clear();
	REQUIRE(Scene.GetBodies().Num() == 0);
}
...
TEST_CASE("PhysScene QuerySupportY uses body tops", "[physics][physscene]")
{
	...
	const float Support = Scene.QuerySupportY(Capsule, {0.0f, 2.1f, 0.0f}, 0.0f, 0.35f, 0.02f, ULevel::Npos);
...
TEST_CASE("PhysScene ApplyCapsuleSweepPush moves Dynamic without penetration", "[physics][physscene][push]")
{
	FPhysScene Scene;

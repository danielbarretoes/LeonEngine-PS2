...
namespace
{
	...
	[[nodiscard]] bool SegmentFloorY(
		const glm::vec3& Start, const glm::vec3& End, float FloorY, float& OutT, glm::vec3& OutNormal)
	{
	...
	[[nodiscard]] bool PointInAabb(
		const glm::vec3& P, const glm::vec3& Center, const glm::vec3& HalfExtents, float Inflate)
	{
		const glm::vec3 He = HalfExtents + glm::vec3{Inflate};
		return std::abs(P.x - Center.x) <= He.x && std::abs(P.y - Center.y) <= He.y && std::abs(P.z - Center.z) <= He.z;
	}
	...
	[[nodiscard]] bool SegmentSlopePlane(const glm::vec3& Start, const glm::vec3& End, const FSlopePlane& Plane,
	...
	void WriteHit(FHitResult& Out, const glm::vec3& Start, const glm::vec3& End, float T, const glm::vec3& Normal,
		std::size_t LevelMeshIndex, bool bFloorPlane)
	{
	...
	void AppendSlopePlaneHits(std::vector<FHitResult>& OutHits, const glm::vec3& Start, const glm::vec3& End,
		float Radius, float HalfHeight, const std::vector<FSlopePlane>& Planes)
	{
	...
	{
		std::sort(Hits.begin(), Hits.end(), [](const FHitResult& A, const FHitResult& B) { return A.Time < B.Time; });
	}

	/// Copy nearest Multi hit into `outHit` (Unreal Single returns the first blocking hit).
	[[nodiscard]] bool TakeNearestHit(
		const std::vector<FHitResult>& Hits, FHitResult& OutHit, const glm::vec3& Start, const glm::vec3& End)
	{
		OutHit = {};
	...
	void AddRingXz(FDebugDraw& Draw, const glm::vec3& Center, float Radius, const glm::vec3& Color, int Segments = 16)
	{
		...
		for (int I = 0; I < Segments; ++I)
		{
			...
			const float A1 = (static_cast<float>(I + 1) / SegCount) * (2.0f * std::numbers::pi_v<float>);
	...
	{
		const float S = 0.08f;
		Draw.AddLine(Hit.ImpactPoint + glm::vec3{-S, 0, 0}, Hit.ImpactPoint + glm::vec3{S, 0, 0}, TraceNormal);
		Draw.AddLine(Hit.ImpactPoint + glm::vec3{0, -S, 0}, Hit.ImpactPoint + glm::vec3{0, S, 0}, TraceNormal);
		Draw.AddLine(Hit.ImpactPoint + glm::vec3{0, 0, -S}, Hit.ImpactPoint + glm::vec3{0, 0, S}, TraceNormal);
...
bool FPhysScene::LineTraceMultiByChannel(std::vector<FHitResult>& OutHits, const glm::vec3& Start, const glm::vec3& End,
...
bool FPhysScene::LineTraceSingleByChannel(FHitResult& OutHit, const glm::vec3& Start, const glm::vec3& End,
	ECollisionChannel Channel, const FCollisionQueryParams& Params, FDebugDraw* DebugDraw) const
{
...
bool FPhysScene::SphereTraceSingleByChannel(FHitResult& OutHit, const glm::vec3& Start, const glm::vec3& End,
...
bool FPhysScene::CapsuleTraceMultiByChannel(std::vector<FHitResult>& OutHits, const glm::vec3& Start,
	...
	const FCollisionQueryParams& Params, FDebugDraw* DebugDraw) const
{
	const float R = std::max(Radius, 0.0f);
	const float Hh = std::max(HalfHeight, 0.0f);
	OutHits.clear();
...
bool FPhysScene::CapsuleTraceSingleByChannel(FHitResult& OutHit, const glm::vec3& Start, const glm::vec3& End,

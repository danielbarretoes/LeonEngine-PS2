...
bool FLevelDirector::ScanPackAndLoad(
	UGameEngine& Engine, const std::string& PackDirectory, std::string_view PreferredLevelKey)
{
	...
	if (!PreferredLevelKey.empty())
	{
		...
		std::cerr << "LevelDirector: defaultLevel '" << PreferredLevelKey << "' not in catalog; loading first entry\n";
	}
	return LoadIndex(Engine, 0);
}
...
void FLevelDirector::Update(UGameEngine& Engine, float DeltaTime)
{
	...
	for (const auto& Spin : Animation.Spins)
	{
		...
		{
			Objects[Spin.MeshIndex].Transform.RotationDegrees.y += Spin.YawDegreesPerSec * DeltaTime;
		}
	}
	...
	for (const auto& Orbit : Animation.Orbits)
	{
		...
		{
			Points[Orbit.LightIndex].Transform.Position.x = std::cos(Elapsed * Orbit.Speed) * Orbit.Radius;
			Points[Orbit.LightIndex].Transform.Position.z = std::sin(Elapsed * Orbit.Speed) * Orbit.Radius;
...
void FLevelDirector::LayoutChrome(int FramebufferWidth, int FramebufferHeight)
{
	...
	if (std::snprintf(Label.data(), Label.size(), "<  %s/%s  (%zu/%zu)  >",
			Entry.Pack.empty() ? "-" : Entry.Pack.c_str(), Entry.Name.c_str(), CurrentIndex + 1,
	...
	const float OriginY = static_cast<float>(std::max(FramebufferHeight, 1)) - Margin - LineHeight;
...
bool FLevelDirector::HandleUiInput(UGameEngine& Engine)
{
	...
	{
		const bool bDown = Engine.GetWindow().IsKeyPressed(static_cast<EKeys>(ToKeyCode(EKeys::One) + Digit)) ||

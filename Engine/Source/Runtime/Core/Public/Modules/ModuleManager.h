...
public:
	...
	/**
	 * The module's interface as TModuleInterface, or null when the target does not link it (UE: GetModulePtr). Every
	 * module is statically linked and started with the table, so nothing is loaded here.
	 */
	template <typename TModuleInterface>
	static TModuleInterface* GetModulePtr(const char* ModuleName)
	{
		return static_cast<TModuleInterface*>(Get().GetModule(ModuleName));
	}

	/** The module's interface as TModuleInterface; a module the target does not link is a fatal error (UE). */
	template <typename TModuleInterface>
	static TModuleInterface& LoadModuleChecked(const char* ModuleName)
	{
		IModuleInterface* Module = Get().GetModule(ModuleName);
		checkf(Module != nullptr, "LoadModuleChecked: module '%s' is not linked into this target", ModuleName);

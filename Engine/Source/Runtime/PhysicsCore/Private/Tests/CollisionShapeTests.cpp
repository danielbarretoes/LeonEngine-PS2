...
TEST_CASE("HalfExtentsFromScale uses absolute half scale", "[physics][collision]") {
    ...
    leon::HalfExtentsFromScale({2.0f, -4.0f, 6.0f}, hx, hy, hz);
...
TEST_CASE("MassFromHalfExtents floors tiny volumes", "[physics][collision]") {
    REQUIRE(leon::MassFromHalfExtents(0.01f, 0.01f, 0.01f) >= 0.08f);
    REQUIRE_THAT(leon::MassFromHalfExtents(1.0f, 1.0f, 1.0f), WithinAbs(8.0f, 1.0e-5f));
}

TEST_CASE("ClampPositionXZ clamps to bounds", "[physics][collision]") {
    glm::vec3 p{100.0f, 5.0f, -50.0f};
    leon::ClampPositionXZ(p, 18.0f);
...
TEST_CASE("XzDiscOverlapsAabb detects overlap", "[physics][collision]") {
    REQUIRE(leon::XzDiscOverlapsAabb(0.0f, 0.0f, 0.5f, 0.0f, 0.0f, 0.5f, 0.5f, 0.0f));
    REQUIRE_FALSE(leon::XzDiscOverlapsAabb(5.0f, 0.0f, 0.3f, 0.0f, 0.0f, 0.5f, 0.5f, 0.0f));
}

TEST_CASE("CapsuleAabbMtv pushes capsule out of AABB", "[physics][collision]") {
    glm::vec2 normal{};
    float penetration = 0.0f;
    REQUIRE(leon::CapsuleAabbMtv(0.0f, 0.0f, 0.5f, 0.0f, 0.0f, 0.4f, 0.4f, normal, penetration));
    REQUIRE(penetration > 0.0f);
    REQUIRE(glm::length(normal) > 0.5f);

    REQUIRE_FALSE(
        leon::CapsuleAabbMtv(3.0f, 0.0f, 0.3f, 0.0f, 0.0f, 0.4f, 0.4f, normal, penetration));
}
...
TEST_CASE("SeparateAabb separates overlapping boxes", "[physics][collision]") {
    ...
    glm::vec3 normal{};
    REQUIRE(leon::SeparateAabb(a, half, b, half, 0.5f, 0.5f, &normal));

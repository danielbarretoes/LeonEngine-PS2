...
void UCharacterAnimInstance::UpdateJumpStateMachine() {
    const bool bHasJumpStart =
        JumpClips.JumpStart != nullptr && JumpClips.JumpStart->FrameCount() > 0;
    const bool bHasFallLoop =
        JumpClips.FallLoop != nullptr && JumpClips.FallLoop->FrameCount() > 0;
        ...
        if (bJustLanded || !bFalling) {
            if (bHasLand) {
                ...
                EnterState(EAnimJumpState::Locomotion);
                   ...
                   (Active.Sequence != nullptr && Active.Sequence->IsFinished(Active.Time))) {
            if (bHasFallLoop) {
                EnterState(EAnimJumpState::FallLoop);
            }
        }
    ...
    case EAnimJumpState::Land:
        ...
        } else if (Active.Sequence == nullptr || Active.Sequence->IsFinished(Active.Time) ||
                   !bHasLand) {
            bJumpRequested = false;
            EnterState(EAnimJumpState::Locomotion);
        } else {
...
void UCharacterAnimInstance::NativeUpdateAnimation(float DeltaTime) {
    ...
    if (CrossfadeAlpha < 1.0f) {
        const float FadeDur =
            ActiveCrossfadeDuration > 1.0e-6f ? ActiveCrossfadeDuration : CrossfadeDuration;
        if (FadeDur <= 1.0e-6f) {
            CrossfadeAlpha = 1.0f;

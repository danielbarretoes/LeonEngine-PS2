...
cmake -S Editor -B Editor/build-ninja -G Ninja -DCMAKE_BUILD_TYPE=Release -DCMAKE_EXPORT_COMPILE_COMMANDS=ON || exit /b 1
cmake -S Tools -B Tools/build-ninja -G Ninja -DCMAKE_BUILD_TYPE=Release -DCMAKE_EXPORT_COMPILE_COMMANDS=ON || exit /b 1
cmake --build Editor/build-ninja -j %NUMBER_OF_PROCESSORS% || exit /b 1
...
echo clangd: Editor/build-ninja + Tools/build-ninja (see .clangd / Docs/SETUP.md)
endlocal

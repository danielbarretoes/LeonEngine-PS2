...
| `RunTests.bat` | `RunTests.bat [-automation=<filter>]` | Builds `LeonAutomationTests Win64 Development` and runs `Engine\Binaries\Win64\LeonAutomationTests.exe` from the repo root: every automation test (383), or only those whose name contains `<filter>`. It then runs the LeonHeaderTool golden tests (`LeonHeaderTool -Test`), then builds `ShooterGameTests` (`-Project=Game\ShooterGame\ShooterGame.lproj`) and runs `Game\ShooterGame\Binaries\Win64\ShooterGameTests.exe` with the same arguments (ShooterGame's tests, 19) |

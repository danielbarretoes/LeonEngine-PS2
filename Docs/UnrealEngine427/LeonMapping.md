...
| Game mode choice | `UWorld::SetGameMode(FURL)` asks the game instance (`?game=`, world settings, `GlobalDefaultGameMode`) | `UWorld::SetGameMode(TSubclassOf<AGameModeBase>)`; `FGameApplication` passes `ADefaultGameMode` | no `FURL` or world settings before P13 (D18) |
...
| Render state | `UPrimitiveComponent::CreateRenderState_Concurrent` adds a scene proxy to `FScene` | adds the component to `UWorld`'s primitive list; `UWorld::SubmitPrimitiveDraws` asks each visible one to `SubmitDraw` | the `FScene` / `IRendererModule` boundary is P13 |
| Physics state | components create `FBodyInstance`s (`CreatePhysicsState`) | `CreatePhysicsState` is a hook only: bodies still come from the `.llev` meshes (`UWorld::RegisterBodiesFromLevel`) and the character capsule is swept, not simulated | levels become actors in P13 |

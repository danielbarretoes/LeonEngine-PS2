...
| Legacy content | — | P14 migrated it: `MigrateLegacyContent` (LeonEd, temporary) converts `.lmat` / `.lmesh` / image / `.wav` files into packages next to them, named by the `.llev` content keys (`FLegacyAssetKeys`: prefix, engine folders renamed to `EngineMaterials`), and the `.llev` reader resolves its keys to those packages, with a mount point named after a content folder outside the mount points | the `.llev` levels stay until P15 |
...
| Basic shapes | `/Engine/BasicShapes` are imported meshes | saved once from Leon's procedural generators; a `.llev` sphere of another tessellation is built at run time | identical frames: the generator's geometry, bit for bit |
...
| Legacy data | assets are in UE space | `.llev` files stay Y up in metres and are converted by `FLegacyCoordinateConversion` in their reader (the level saver converts back); G4 keeps the converter out of every other file | existing content keeps loading; the `.llev` levels go with the `.lmap` maps (P15) |
...
| `FName` in archives | an index into the package name table (the base `FArchive` does not store names) | the package linkers write the name table index and the number, as UE; the base `FArchive` writes a string | `.llev` and the tests serialize names through plain archives |

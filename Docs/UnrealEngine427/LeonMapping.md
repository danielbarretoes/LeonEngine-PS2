...
| — | `CoreUObject` (P9–P11) | UE `Runtime/CoreUObject`: `UObject`, reflection, `NewObject`, the object array, garbage collection, references, config and Exec, packages (`.lasset` / `.lmap`, linkers, `LoadObject`); every platform; only its own types and test fixtures are reflected until P12 |

...
| --- | --- | --- | --- |
| Reflection | `UCLASS`, `UObject`, UHT | LeonHeaderTool generates UE 4.27-shaped `.generated.h` / `.gen.cpp` (no metadata, no hot-reload CRCs, explicit `RegisterReflection_<Module>` instead of static `FCompiledInDefer` objects) and CoreUObject runs it (P9); the gameplay framework, the widgets and the anim instances are reflected since P12, the level content actors, the engine, the viewport client, the players and the settings since P13; the mesh, texture and animation assets, `UNavigationSystem` and the behavior tree lite are still plain C++ with U names until P14 | [LeonHeaderTool/README.md](../../Engine/Source/Programs/LeonHeaderTool/README.md), [CoreUObject/README.md](../../Engine/Source/Runtime/CoreUObject/README.md) |

...
| — | `CoreUObject` (P9–P11) | UE `Runtime/CoreUObject`: `UObject`, reflection, `NewObject`, the object array, garbage collection, references, config and Exec, packages (`.lasset` / `.lmap`, linkers, `LoadObject`); every platform; since P12 Engine, AIModule, UMG and AnimationCore are reflected on it |
...
| `Texture`, `StaticMesh`, `SkeletalMesh`, `Material` | `UTexture2D`, `UStaticMesh`, `USkeletalMesh` (Engine CPU data since P13; the GPU copies are the Renderer's), `FMaterial` (render parameters; no `UMaterial` asset class yet) |
...
| --- | --- | --- | --- |
| Reflection | `UCLASS`, `UObject`, UHT | LeonHeaderTool generates UE 4.27-shaped `.generated.h` / `.gen.cpp` (no metadata, no hot-reload CRCs, explicit `RegisterReflection_<Module>` instead of static `FCompiledInDefer` objects) and CoreUObject runs it (P9); the gameplay framework, the widgets and the anim instances are reflected since P12, the level content actors since P13; `UGameEngine`, the mesh, texture and animation assets, `UNavigationSystem` and the behavior tree lite are still plain C++ with U names until P13 / P14 | [LeonHeaderTool/README.md](../../Engine/Source/Programs/LeonHeaderTool/README.md), [CoreUObject/README.md](../../Engine/Source/Runtime/CoreUObject/README.md) |
| Engine object | `UEngine` / `UGameEngine` UObjects, `GEngine`, `UEngine::WorldList` of `FWorldContext`s | `UGameEngine` is plain C++ and an `FGCObject` (it reports its game instance, camera, HUD and player input); each `UGameInstance` owns its `FWorldContext` | P13 brings `UEngine` / `UGameEngine`, `GEngine` and `LoadMap` |
| World package and play | `LoadMap` loads the world from its `.umap` package and calls `BeginPlay` when the map is ready | `UWorld::CreateWorld` makes a transient `/Temp/Untitled_<N>` package and the world plays at once (actors begin play when spawned) | no map packages before P15; P13's `LoadMap` sequences `BeginPlay` |
| Game mode choice | `UWorld::SetGameMode(FURL)` asks the game instance (`?game=`, world settings, `GlobalDefaultGameMode`) | `UWorld::SetGameMode(TSubclassOf<AGameModeBase>)`; `FGameApplication` passes `ADefaultGameMode`; the level's game mode string is kept on `AWorldSettings` but not read | no `FURL` yet; D18's precedence comes with `LoadMap` (P13, second part) |
| Game mode hooks | the game mode runs from `InitGame`, `Login`, `StartPlay` and its actor tick | Leon's engine hooks stay: `OnEnter(Engine, LevelPath)`, `Tick(Engine, DeltaTime)` (which ticks the world), `OnExit(Engine)`, called by `FGameApplication` | P13 replaces them with `UEngine::LoadMap` and the UE flow |
...
| Actor tick | `FActorTickFunction` / `FActorComponentTickFunction` registered with the world's tick task manager, tick groups | `UWorld::Tick` walks the level: each actor ticks its registered components that enabled their tick, then `Tick` (`bCanEverTick`; `AInfo` actors do not tick) | one thread, no tick groups or prerequisites yet |
...
| GPU resources | `UStaticMesh::RenderData`, `UTexture2D::Resource`: the asset owns its render resource | the Renderer's private `FRenderResourceCache` keeps the GPU copy of each CPU asset (a shared pointer to it pins the asset) and makes it the first time a proxy or a material uses it; Engine keeps `FResourceCache`, its CPU loader, until the P14 assets | Engine must not include Renderer headers, and the assets are not UObjects yet |
| Renderer interface | `IRendererModule` has no renderer start-up, stats or read-back | Leon's `IRendererModule` adds `InitRenderer` / `ShutdownRenderer` (the GL objects on the window's context), `ReloadShaders`, `GetFrameStats`, `ReadFramebufferBgr` (screenshots) and `DrawCanvas` (UE draws the canvas through its render items) | no RHI command lists or `FViewport` yet |
| Scene view | `FSceneViewInitOptions` takes the view origin and rotation, and the FOV is horizontal | the view takes the camera's whole view matrix, projection and vertical FOV (`FSceneView::FromCamera`); the show flags hold only `Bounds` (F1) and Leon's `AxesGizmo` (F6) | Leon's camera builds its own matrices (orbit and free look); see Field of view |
...
| HUD and input ownership | the player controller spawns its `AHUD` and creates its `UPlayerInput` from the input settings | the engine owns one `AHUD` (outside any world) and one `UPlayerInput` fed by `UInputMappingContext` values | P13: viewport client, input by config |
...
| Config usage | `UPROPERTY(Config)` / `LoadConfig` everywhere, input from `BaseInput.ini` | `UPROPERTY(Config)` / `LoadConfig` / `SaveConfig` work (P10) and the gameplay classes are UObjects (P12), but the engine still reads only a few keys by hand (map, resolution, stats, the collection interval, ThirdPerson tuning) | config-driven engine classes and input come in P13 |

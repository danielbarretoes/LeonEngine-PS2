...
| `UserWidget`, `TextBlockWidget`, `ButtonWidget`, … | `UUserWidget`, `UTextBlock`, `UButton` (removed in 0.20.1: Leon's widgets take no input), … |
...
| | `Shader`, `ShadowMap`, `GpuPassTimer`, `LdrColorTarget`, `SsaoTarget`, … | `FShader`, `FShadowMap`, `FGPUPassTimer`, `FLDRColorTarget`, `FSSAOTarget` (both removed in 0.20.1 with the post-process stack), … |
...
| 4.5 UMG | `ButtonWidget`, `ImageWidget`, `ProgressBarWidget`, `TextBlockWidget`, `VerticalBoxWidget`, `WidgetPaintContext` | `UButton`, `UImage`, `UProgressBar`, `UTextBlock`, `UVerticalBox` (files renamed to match; rebuilt as UE's widget tree in 0.20.1, when `UButton` went), `FPaintContext` |
...
| `FLegacyAssetLoader::LoadEngineObject`, `GetSphereMesh` | `LoadObject` (`UEngine::InitializeObjectReferences`, `UMaterial::GetDefaultMaterial`, `MeshForBasicShape`), `GetSphereMesh` (the package for 24 × 16, else built at run time); `MeshForBasicShape` and `GetSphereMesh` were removed in 0.20.1 (code loads `/Engine/BasicShapes/*` with `LoadObject`) | `Engine/Private/UnrealEngine.cpp`, `Materials/Material.cpp`, `Level/BasicShape.cpp` (removed) |
...
| `ULegacyLevelDataComponent`: spin, bob, trigger data, light orbit, game mode string, level name, camera framing | `URotatingMovementComponent` (UE's); Leon's `UBobbingMovementComponent`, `UOrbitMovementComponent`, `UInteractableComponent` (removed in 0.20.1: no UE class, no map used them); `AWorldSettings::DefaultGameMode`; the map's name; `ACameraActor` (UE's) holding the framing, and an `APlayerStart` at the view it opened with | `Engine/Classes/GameFramework/*MovementComponent.h`, `Components/InteractableComponent.h`, `Camera/CameraActor.h` |
...
The code without a UE counterpart goes, the post-process stack goes (the PS2 GS has no programmable pixel stage), and
UMG takes UE's widget tree. Details: [ARCHITECTURE.md](../ARCHITECTURE.md),
[ShooterGame README — Bots](../../Game/ShooterGame/README.md#bots).

| Leon (before) | UE name (now) | Where |
| --- | --- | --- |
| `UBobbingMovementComponent`, `UOrbitMovementComponent`, `UInteractableComponent`, `VolumeHelpers`, `FAIChaseBehavior`, `FBasicShape`, `FBasicLight`, `Content/LevelClassNames.cpp`, the `GameplayMinimal.h` and `PhysicsMinimal.h` umbrella headers, `UMenuListWidget`, `UInteractionPromptWidget` | removed (no UE counterpart) | — |
| `AAIController`: `EAILogicState`, `SetLogicState` / `GetLogicState`, `SetWishDirection` / `ClearWishDirection` | `EPathFollowingStatus::Type`, `GetMoveStatus()` (UE's path following status) | `AIModule/Classes/AIController.h` |
| `AGameModeBase::PrepareMatchWorld`, `RebuildNavigation`, `SnapCharacterToFloor`, `EstimateFloorZ`, `EstimateWalkBounds`, `SetPhysicsBackend`; `AGameStateBase::MapName`, `ReplicatedWorldTimeFrames`; `APlayerState::Lives`; `CreateShared` of `FGenericWindow` / `FGLFWWindow`; `UGameplayStatics::SweepCapsuleAlongSegment`; `EMeshDataBasis` | removed (unused) | — |
| `APainCausingVolume`: data only (gameplay code applied it through `VolumeHelpers`) | `CausePainTo` every `PainInterval` (`DamagePerSec` × `PainInterval` to each pawn it encompasses); the volume ticks | `Engine/Classes/GameFramework/PainCausingVolume.h` |
| `UNavigationSystem::FindPath(Start, End, OutPath)`; `AAIController` jumped only onto a rise | `FindPath(Start, End, OutPath, OutNodes)` (each point's node); `AAIController` jumps near a `Jump` waypoint and crouches along the links of a `Crouch` one (`IsCrouchedForPath`) | `Engine/Public/AI/Navigation/NavigationSystem.h`, `AIModule/Classes/AIController.h` |
| `UMapImportSettings`' `Waypoint*` agent properties | `[/Script/Engine.NavigationSystem]` of the Engine config (`FWaypointLinkParams::FromConfig`), read by the map import and the world's graph | `Engine/Public/AI/Navigation/NavigationSystem.h` |
| `FWorldGameplayFrameParams::NavMeshDebugDraw`; `show Collision` / `show Navigation` not drawn | `CollisionDebugDraw`, `NavigationDebugDraw`: `UGameEngine::Tick` passes the world's `LineBatcher` when the viewport's show flags ask (windowed runs) | `Engine/Classes/Engine/World.h`, `Engine/Private/GameEngine.cpp` |
| `FPostProcessSettings`, `EPostProcessQuality`, `FSceneColorTarget`, `FLDRColorTarget`, `FSSAOTarget`, early-Z, the `ssao` / `ssao_blur` / `fxaa` / `post_composite` / `fullscreen` shaders; `SetDrawFramebuffer`, `SetSceneGeometryEnabled` | removed: the preview renders straight to the back buffer; `FSceneRenderer::ReadRendererSettings` reads `r.ShadowMapResolution` / `r.PlanarReflectionScale` from `[/Script/Engine.RendererSettings]`; `FGPUPassTimer` times Shadow, Planar and Color | `Renderer/Private/SceneRenderer.h`, `Renderer/Public/GPUPassTimer.h`, `Engine/Config/BaseEngine.ini` |
| `[/Script/Engine.PhysicsSettings] DefaultPhysicsBackend` (never read) | removed: the default scene is Arcade (the PS2 runs Arcade only) | `Engine/Config/BaseEngine.ini` |
| `UButton`, the menu-style `UVerticalBox` | removed; UE's tree: `UWidget` (`ESlateVisibility`, `Slot`, `GetDesiredSize`, `Paint`), `UPanelWidget` / `UPanelSlot` (`AddChild`, `RemoveChild`, `ClearChildren`, `GetChildAt`, `GetChildIndex`, `HasChild`), `UContentWidget`, `UBorder`, `UVerticalBox` / `UVerticalBoxSlot`, `UCanvasPanel` / `UCanvasPanelSlot`, `UTextBlock`, `UImage`, `UProgressBar`; `UUserWidget` (a `UWidget`) with a `UWidgetTree` (`ConstructWidget`, `RootWidget`, `Initialize` → `NativeOnInitialized`) | `UMG/Public/Components/`, `UMG/Public/Blueprint/` |
...
| Basic shapes | `/Engine/BasicShapes` are imported meshes | saved once from Leon's procedural generators | identical frames: the generator's geometry, bit for bit |
...
| Scene view | `FSceneViewInitOptions` takes the view origin and rotation, and the FOV is horizontal | the view takes the camera's whole view matrix, projection and vertical FOV (`FSceneView::FromCamera`); the show flags hold `Bounds` (F1), `Collision` (F2), `Navigation` (F3) and Leon's `AxesGizmo` (F6); `Collision` and `Navigation` draw into the world's `LineBatcher` through the gameplay frame (`FWorldGameplayFrameParams::CollisionDebugDraw` / `NavigationDebugDraw`) in windowed runs | Leon's camera builds its own matrices (orbit and free look); see Field of view |
...
| Map data with no UE class | — | `ANavigationWaypoint` (the waypoint graph, P20); the legacy levels' bob, light orbit and trigger data components (P15) were removed in 0.20.1 | UE has no waypoint actor; no map used the others |
...
| Containers / strings | `TArray`, `TMap`, `FString` everywhere | the same everywhere since P6, enforced by `CheckBannedApis.ps1` (G4); third-party types stay at the library seams (Jolt, tinyobjloader, ufbx, cgltf) | — |
| `FString` comparison | `==` ignores case | the same; code that needs an exact match (the level string table, editor class names) calls `Equals(…, ESearchCase::CaseSensitive)`; actor tags are `FName`s since P13 (see Actor tags) | the pre-P6 `std::string` code compared case-sensitively |
...
| Post processing | post process volumes and settings, tonemapper, anti-aliasing, ambient occlusion, HDR scene color, scalability groups and `r.*` console variables | none: the Win64 preview renders straight to the back buffer; `r.ShadowMapResolution` (1024, 512..4096) and `r.PlanarReflectionScale` (0.5, 0.1..1) are read once from `[/Script/Engine.RendererSettings]` (not console variables) | the PS2 GS has no programmable pixel stage, so the preview does not fake one |
...
| Config arrays on save | UE tracks per config value whether a key is an array | a section remembers its array keys (`FConfigSection::ArrayKeys`: `SetArray`, `RemoveKey`, or a `+`, `.`, `-` or `!` line); a saved user layer writes each as `!Key=ClearArray` plus one `.Key=Value` line a value, so a one-value array replaces the lower layers' and duplicates survive; `RemoveKey` / `EmptySection` are saved (an empty array in the user layer); `Flush(true)` reloads a global file from its whole hierarchy | a saved layer must replace the arrays below it, whatever their size |
...
| Widgets | UMG widgets wrap Slate widgets (`TakeWidget`), route input (`UButton`, focus) and lay out with every slot option | the widget tree has no Slate behind it: `UWidget::Paint` calls the widget's `OnPaint` / `ComputeDesiredSize` (Slate's names) in the rectangle its slot gives it; widgets take no input (no `UButton`); a vertical box slot always sizes `Auto`, a canvas panel slot's anchors are always the top-left corner; `UBorder` and `UImage` brushes are a plain colour | no Slate on Leon; the HUD's widgets only display game state |

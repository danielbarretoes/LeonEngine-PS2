...
Shell equivalents: `Engine/Build/BatchFiles/Linux/Build.sh` (it also builds inside the ps2dev container), `Engine/Build/BatchFiles/Linux/GenerateProjectFiles.sh`, root `GenerateProjectFiles.sh` and `Setup.sh`. Linux is not an official platform: Win64 is the development and editor platform, PS2 the target. A PS2 Docker build started from a Unix host runs the container as root and gives the outputs back to the calling user (`LEON_HOST_UID` / `LEON_HOST_GID`, [BUILD.md](BUILD.md#ps2-builds-in-docker)).

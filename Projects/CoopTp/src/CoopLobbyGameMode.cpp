...
void CoopLobbyGameMode::rebuildMenu(leon::Engine& engine) {
    if (menu_ == nullptr) {
        return;
    }
    ...
    menu_->ClearChildren();
    if (host) {
        menu_->AddButton("map_courtyard",
                         std::string("Courtyard") +
                             (session.GetMatchMapName() == CoopGameInstance::kDefaultMatchMap
                                  ? "  [selected]"
                                  : ""));
        menu_->AddButton("map_rooftops",
                         std::string("Rooftops") +
                             (session.GetMatchMapName() == CoopGameInstance::kAltMatchMap
                                  ? "  [selected]"
                                  : ""));
        menu_->AddButton("travel_match", "Start Match");
...
void CoopLobbyGameMode::NotifyClientsServerTravel(leon::Engine& engine, std::string_view mapName) {
    leon::net::SendTravelToPeers(engine.GetGameInstance().GetNetDriver(), mapName,
                                 engine.GetGameInstance().IsDedicatedServer());
}

void CoopLobbyGameMode::ServerTravelToMatchMap(leon::Engine& engine) {
    CoopGameInstance& session = Session(engine);
    const std::string mapName = session.GetMatchMapName().empty()
                                    ? std::string(CoopGameInstance::kDefaultMatchMap)
                                    : session.GetMatchMapName();
    session.SetMatchMapName(mapName);
    NotifyClientsServerTravel(engine, mapName);
    (void)ServerTravel(engine, mapName, levelPath_);
}

void CoopLobbyGameMode::SendClientHello(leon::Engine& engine) {
    leon::net::HelloMsg hello{};
    engine.GetGameInstance().GetNetDriver().SendToPeer(&hello, sizeof(hello), true);
    std::cout << "CoopLobby: client Hello sent\n";
}

void CoopLobbyGameMode::setupNetCallbacks(leon::Engine& engine) {
    ...
    net.SetOnPeerDisconnected([this, &engine](int /*peerSlot*/) {
        if (engine.GetGameInstance().IsClient()) {
            ...
            std::cout << "CoopLobby: disconnected -> MainMenu\n";
...
void CoopLobbyGameMode::handlePacket(leon::Engine& engine, int peerSlot, const std::uint8_t* data,
                                     std::size_t size) {
    ...
    if (type == leon::net::ENetMsg::Hello && gi.IsNetHost() &&
        ...
        welcome.slot = gi.IsDedicatedServer() ? static_cast<std::uint8_t>(peerSlot)
                                              : static_cast<std::uint8_t>(1);
        leon::net::WriteLevelKey(welcome.levelKey, CoopGameInstance::kLobbyMap);
    ...
    if (type == leon::net::ENetMsg::Welcome && gi.IsClient() &&
        ...
        if (!hostLevel.empty() && !LevelKeysEqual(hostLevel, here)) {
            if (!LevelKeysEqual(hostLevel, CoopGameInstance::kLobbyMap) &&
                !LevelKeysEqual(hostLevel, CoopGameInstance::kMainMenuMap)) {
                session.SetMatchMapName(hostLevel);
            }
            std::cout << "CoopLobby: Welcome travel '" << here << "' -> '" << hostLevel << "'\n";
    ...
    if (type == leon::net::ENetMsg::Travel && gi.IsClient() &&
        ...
        std::cout << "CoopLobby: Travel '" << here << "' -> '" << session.GetMatchMapName()
                  << "'\n";
        applyingTravel_ = true;
...
void CoopLobbyGameMode::OnEnter(leon::Engine& engine, const std::string& levelPath) {
    if (applyingTravel_) {
        return;
    }
    // Flow: Lobby -- pick map / wait; dedicated headless auto ServerTravelToMatchMap.
    engine_ = &engine;
    ...
    if (session.GetMatchMapName().empty()) {
        session.SetMatchMapName(CoopGameInstance::kDefaultMatchMap);
    }
    ...
    std::cout << "CoopLobby: map='" << session.GetMatchMapName() << "' mode="
...
void CoopLobbyGameMode::OnExit(leon::Engine& engine) {
...
void CoopLobbyGameMode::activate(leon::Engine& engine, const std::string& itemId) {
    CoopGameInstance& session = Session(engine);
    if (itemId == "menu") {
        std::cout << "CoopLobby: leave session -> MainMenu\n";
    ...
    if (itemId == "map_courtyard" && engine.GetGameInstance().IsNetHost()) {
        session.SetMatchMapName(CoopGameInstance::kDefaultMatchMap);
        rebuildMenu(engine);
        return;
    }
    if (itemId == "map_rooftops" && engine.GetGameInstance().IsNetHost()) {
        session.SetMatchMapName(CoopGameInstance::kAltMatchMap);
        rebuildMenu(engine);
        return;
    }
...
void CoopLobbyGameMode::Tick(leon::Engine& engine, float deltaTime) {
    ...
    if (engine.GetGameInstance().IsClient() && !session.IsClientWelcomed()) {
        ...
        if (clientConnectSeconds_ >= kWelcomeTimeoutSeconds) {
            std::cout << "CoopLobby: Welcome timeout -> MainMenu\n";
            engine.GetGameInstance().CloseNetSession();
            session.ResetSession();

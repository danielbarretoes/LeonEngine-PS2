...
void ZombiesLobbyGameMode::rebuildMenu(leon::Engine& engine) {
    if (menu_ == nullptr) {
        return;
    }
    ...
    menu_->ClearChildren();
    if (host) {
        menu_->AddButton("travel_match", "Start Match (Town)");
...
void ZombiesLobbyGameMode::NotifyClientsServerTravel(leon::Engine& engine,
                                                     std::string_view mapName) {
    leon::net::SendTravelToPeers(engine.GetGameInstance().GetNetDriver(), mapName,
                                 engine.GetGameInstance().IsDedicatedServer());
}

void ZombiesLobbyGameMode::ServerTravelToMatchMap(leon::Engine& engine) {
    ZombiesGameInstance& session = Session(engine);
    const std::string mapName = ZombiesGameInstance::kDefaultMatchMap;
    session.SetMatchMapName(mapName);
    NotifyClientsServerTravel(engine, mapName);
    (void)ServerTravel(engine, mapName, levelPath_);
}

void ZombiesLobbyGameMode::SendClientHello(leon::Engine& engine) {
    leon::net::HelloMsg hello{};
    engine.GetGameInstance().GetNetDriver().SendToPeer(&hello, sizeof(hello), true);
    std::cout << "ZombiesLobby: client Hello sent\n";
}

void ZombiesLobbyGameMode::setupNetCallbacks(leon::Engine& engine) {
    ...
    net.SetOnPeerDisconnected([this, &engine](int /*peerSlot*/) {
        if (engine.GetGameInstance().IsClient()) {
            ...
            std::cout << "ZombiesLobby: disconnected -> MainMenu\n";
...
void ZombiesLobbyGameMode::handlePacket(leon::Engine& engine, int peerSlot,
    ...
    if (type == leon::net::ENetMsg::Hello && gi.IsNetHost() &&
        ...
        leon::net::WriteLevelKey(welcome.levelKey, ZombiesGameInstance::kLobbyMap);
    ...
    if (type == leon::net::ENetMsg::Welcome && gi.IsClient() &&
        ...
        if (!hostLevel.empty() && !LevelKeysEqual(hostLevel, here)) {
            if (!LevelKeysEqual(hostLevel, ZombiesGameInstance::kLobbyMap) &&
                !LevelKeysEqual(hostLevel, ZombiesGameInstance::kMainMenuMap)) {
                session.SetMatchMapName(hostLevel);
            }
            std::cout << "ZombiesLobby: Welcome travel '" << here << "' -> '" << hostLevel << "'\n";
    ...
    if (type == leon::net::ENetMsg::Travel && gi.IsClient() &&
        ...
        std::cout << "ZombiesLobby: Travel '" << here << "' -> '" << session.GetMatchMapName()
                  << "'\n";
        applyingTravel_ = true;
...
void ZombiesLobbyGameMode::OnEnter(leon::Engine& engine, const std::string& levelPath) {
    if (applyingTravel_) {
        return;
    }
    ...
    if (session.GetMatchMapName().empty()) {
        session.SetMatchMapName(ZombiesGameInstance::kDefaultMatchMap);
    }
    ...
    std::cout << "ZombiesLobby: map='" << session.GetMatchMapName() << "' mode="
...
void ZombiesLobbyGameMode::OnExit(leon::Engine& engine) {
...
void ZombiesLobbyGameMode::activate(leon::Engine& engine, const std::string& itemId) {
    ZombiesGameInstance& session = Session(engine);
    if (itemId == "menu") {
        std::cout << "ZombiesLobby: leave session -> MainMenu\n";
...
void ZombiesLobbyGameMode::Tick(leon::Engine& engine, float deltaTime) {
    ...
    if (engine.GetGameInstance().IsClient() && !session.IsClientWelcomed()) {
        ...
        if (clientConnectSeconds_ >= kWelcomeTimeoutSeconds) {
            std::cout << "ZombiesLobby: Welcome timeout -> MainMenu\n";
            engine.GetGameInstance().CloseNetSession();
            session.ResetSession();

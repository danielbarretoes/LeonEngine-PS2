...
[[nodiscard]] std::string JoinAddressOrLocalhost(const ZombiesGameInstance& session) {
    return session.GetJoinAddress().empty() ? std::string("127.0.0.1") : session.GetJoinAddress();
}

} // namespace

void ZombiesMenuGameMode::rebuildMenu() {
    if (menu_ == nullptr || engine_ == nullptr) {
        return;
    }
    ZombiesGameInstance& session = Session(*engine_);
    ...
    // Dedicated host binary: leon-Zombies-server (or leon-Zombies.exe --dedicated).
    std::string hint = "Arrows / click  |  Enter  |  Esc Quit";
    hint += "\nHost: friends Join Listen your LAN IP";
    hint += "\nJoin Dedicated: needs leon-Zombies-server (or --dedicated) already running";
    if (!session.GetLanAddress().empty()) {
        hint += "\nThis PC LAN: " + session.GetLanAddress() + ":7777";
    }
    hint += "  |  --join <ip>";
    menu_->SetTitle("ZOMBIES");
...
void ZombiesMenuGameMode::syncJoinProgressBar() {
...
void ZombiesMenuGameMode::OnEnter(leon::Engine& engine, const std::string& levelPath) {
    engine_ = &engine;
    ...
    if (engine.GetGameInstance().HasPendingDedicatedStart()) {
        ...
        const char* dest =
            engine.IsHeadless() ? ZombiesGameInstance::kDefaultMatchMap : ZombiesGameInstance::kLobbyMap;
...
void ZombiesMenuGameMode::OnExit(leon::Engine& engine) {
    pendingJoin_ = false;
    pendingJoinSeconds_ = 0.0f;
...
void ZombiesMenuGameMode::cancelPendingJoin(leon::Engine& engine, const std::string& reason) {
    pendingJoin_ = false;
    pendingJoinSeconds_ = 0.0f;
...
void ZombiesMenuGameMode::activate(leon::Engine& engine, const std::string& itemId) {
    ZombiesGameInstance& session = Session(engine);
    if (itemId == "quit") {
        engine.RequestQuit();
        return;
    ...
    if (itemId == "listen") {
        ...
        if (!ServerTravel(engine, ZombiesGameInstance::kLobbyMap, levelPath_)) {
...
void ZombiesMenuGameMode::Tick(leon::Engine& engine, float deltaTime) {
    ...
    if (pendingJoin_) {
        ...
        if (!engine.GetGameInstance().IsClient()) {
            ...
            engine.AddOnScreenDebugMessage("Connected -- entering Lobby...", 2.0f,
                                           {0.4f, 1.0f, 0.55f});
            (void)ClientTravel(engine, ZombiesGameInstance::kLobbyMap, levelPath_);
            return;
